...
![一列定宽，文字环绕效果](/images/css/layout/FixedWidthAndAdaptive-initial.jpg)

不要觉得诧异！因为float最开始的出现就是为了实现这个文字环绕效果的，只不过被攻城师们玩坏了，用到了布局上；但这肯定不是我们想要的，来看看我们想要的是怎么样的吧！
![一列定宽，一列自适应](/images/css/layout/FixedWidthAndAdaptive-target.jpg)
...
一起来看看错误的例子：
![一列定宽，错误](/images/css/layout/FixedWidthAndAdaptive-issue.jpg)
怎么解决呢？
...
大家来看看想要达到的效果：
![二列定宽，一列自适应](/images/css/layout/twoFixedWidthAndAdaptive-initial.jpg)
...
![等分布局分析](/images/css/layout/EqualWidthLayoutThink.jpg)
...
```
实现效果如下：
![等分布局存在左侧padding](/images/css/layout/EqualWidthLayoutPadding.jpg)

效果不是就达到了呢？其实还不尽然，可以看到最左边还有一个宽度，我们不想要怎么办？

这并不是一个bug，而是我们的设置导致的，现在来删除上面的` padding-left: 20px;`吧，可以看到成了这样子：
![等分布局未设置padding](/images/css/layout/EqualWidthLayoutDefault.jpg)
...
```
实现效果如下：
![等分布局FLex](/images/css/layout/EqualWidthLayoutFlex.jpg)
**这里使用的是margin，因为flex自动分配剩余空间**

## 等高布局
大家先来看看想要的效果：
![等高布局](/images/css/layout/EqualHeightLayout.jpg)
